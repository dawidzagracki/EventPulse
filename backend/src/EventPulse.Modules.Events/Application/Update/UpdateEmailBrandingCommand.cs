...
/// </summary>
public sealed record UpdateEmailBrandingCommand(Guid Id, string? AccentColor, string? LogoUrl, string? HeaderName, string? FromName, string? Subject) : IRequest<EventDto>;
...
public sealed class UpdateEmailBrandingValidator : AbstractValidator<UpdateEmailBrandingCommand>
{
    public UpdateEmailBrandingValidator()
    {
        ...
        RuleFor(x => x.FromName).MaximumLength(120);
    ...
    : IRequestHandler<UpdateEmailBrandingCommand, EventDto>
{
    public async Task<EventDto> Handle(UpdateEmailBrandingCommand request, CancellationToken cancellationToken)
    {
        ...
        ev.EmailFromName = string.IsNullOrWhiteSpace(request.FromName) ? null : request.FromName.Trim();
        ev.EmailSubject = string.IsNullOrWhiteSpace(request.Subject) ? null : request.Subject.Trim();
